...
(function(global) {
	...
	Game.prototype = {
		...
		/**
		 * Validate the entire matrix for empty spaces
		 * @returns {Boolean} Full or empty matrix
		 */
		fullMatrix: function() {
			var isValid, val, $element, hasError = false;

			// Go over entire board
			for (var row = 0; row < 9; row++) {
				for (var col = 0; col < 9; col++) {
					val = this.matrix.row[row][col];
					if (val == "") {
	...
	Sudoku.prototype = {
		...
		start: function() {
			...
			if(document.querySelector("#difficultyChoice1") != null && document.querySelector("#difficultyChoice1").checked){
				this.game.config.difficulty = "easy";
			}
			else if(document.querySelector("#difficultyChoice3") != null && document.querySelector("#difficultyChoice3").checked){
				this.game.config.difficulty = "hard";
			}
			else {
				this.game.config.difficulty = "normal";
			}
		 ...
		 */
		reset: function() {
			document.querySelector("#Message").innerHTML = "<H1> </H1>";
			this.game.resetGame();
		},
		...
		validate: function() {
			...
			if (isValid && this.game.fullMatrix() &&!this.isCpuSolved) {
				document.querySelector("#Message").innerHTML = "<H1>you Won !!!</H1>";
...
var game = new Sudoku(".container", null);
